...
def render_role_header(role: str, name: str):
    ...
    role_config = {
        ...
        'student': {'title': 'Learning Hub'}, 
        'teacher': {'title': 'Analytics Dashboard'}, 
        'parent': {'title': 'Student Progress'}
    }
    
    config = role_config.get(role, {'title': 'AERO Assistant'})
    st.markdown(f'<div class="role-header">AERO - {config["title"]} | {name}</div>', 
                unsafe_allow_html=True)
...
def render_document_upload_section(current_user: User, document_service: DocumentService):
    ...
    if current_user and current_user.role == UserRole.ADMIN:
        ...
        st.sidebar.warning("Only administrators can upload documents to the shared knowledge base.")
    ...
    if st.session_state.documents:
        ...
        if current_user and current_user.role == UserRole.ADMIN:
            if st.sidebar.button("Clear Knowledge Base (Admin)"):
...
def render_grounding_info(grounding_result: Dict[str, Any]):
    ...
    st.markdown(f"**Grounding Confidence ({confidence_color}):** {grounding_result['confidence']:.2f}")
